...
const Notifications = {
  ...
  getWhatsappForEmployee: function (emp, settings) {
    // Prefer employerContact phone number if provided
    if (emp.employerContact && emp.employerContact.trim()) {
      return [emp.employerContact.trim().replace(/[^+\d]/g, '')];
    }
    // Fallback to employee's own WhatsApp numbers
